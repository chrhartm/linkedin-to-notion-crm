...
class NotionManager:
    ...
    def ensure_database_exists(self):
        try:
            ...
            logging.info(
                f"Connected to existing database with ID: {self.database_id}")
    ...
    def update_database_properties(self, database):
        try:
                    ...
                    overdue_formula = {
                        'formula': {
                            'expression': ('''
                                ...
                                if(prop("Last Contacted").empty(), false,
            ...
            raise ValueError(
                f"Failed to update database properties. Error: {str(e)}")
        ...
        try:
            properties = {
                "Name": {
                    "title": [{
                        "text": {
                            "content": contact.get("Name", "Unknown")
                        }
                    }]
                },
                "Company": {
            ...
            properties = {
                k: v
                for k, v in properties.items() if v is not None and v != ""
            }
    ...
    def get_all_contacts(self):
        try:
            ...
            while results["has_more"]:
                ...
                db_query_ret = self.client.databases.query(**query_post)
            ...
            logging.info(
                f"Retrieved {len(contacts)} contacts from the database")
            return contacts
        except Exception as e:
